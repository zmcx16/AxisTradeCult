...
class ChartPage(QMainWindow):
    ...
    def setupUIEvent(self):
        ...
        self.parent().ui.ChartGroupsComboBox.currentIndexChanged.connect(
            self.RefreshTechIndicatorsList)
        self.parent().ui.ChartPageStockGroupsComboBox.currentIndexChanged.connect(
        ...
        for key, value in TechIndicatorWidgetParam.items():
            Indicator = TechIndicatorWidget(
                self, key, value, TechIndicatorsWidget_width)
            Indicator.setGeometry(
                QRect(0, 0, Indicator.widget_width, Indicator.widget_height))
            Indicator.setMinimumSize(
                Indicator.widget_width, Indicator.widget_height)
    ...
    def SetStockGroupsComboBoxItem(self):
        ...
        self.parent().ui.ChartPageStockInGroupComboBox.addItems(
            gv.StockGroups[self.parent().ui.ChartPageStockGroupsComboBox.currentText()])
    ...
    @staticmethod
    def SetBackQDate(srcDate, years=0, months=3, days=0):
        pyDate = QDate.toPyDate(srcDate)
        pdDate = pandas.to_datetime(pyDate)
        TargetDate = pdDate - \
            pandas.DateOffset(months=months, days=days, years=years)
        return QDate(TargetDate.year, TargetDate.month, TargetDate.day)

    @staticmethod
    def FormatIndicatorParamString(Indicator):
    ...
    @staticmethod
    def TechIndicatorGroupToFuncDict(TechIndicatorGroup):
        ...
        for TechIndicator in TechIndicatorGroup:
            ...
            OutputTechIndicators.append(
                {strName: IndicatorName, strParam: IndicatorParam})
    ...
    def DoShowButton(self):
        ...
        StartDate = pandas.to_datetime(QDate.toPyDate(
            self.parent().ui.ChartStartDate.date()))
        EndDate = pandas.to_datetime(QDate.toPyDate(
            self.parent().ui.ChartEndDate.date()))
        df = GetStockPriceVolumeData(
            StockSymbol, gv.StockDataPoolPath, StartDate, EndDate)

        PlotType = self.parent().ui.ChartPageGraphTypeComboBox.currentText()

        SelectGroupName = self.parent().ui.ChartGroupsComboBox.currentText()
        TechIndicators = self.TechIndicatorGroupToFuncDict(
            copy.deepcopy(gv.TechIndicatorGroups[SelectGroupName]))
        print(TechIndicators)

        self.graph = ScrollableWindow(PlotStockData(
            StockSymbol, df, PlotType, TechIndicators, gv.SettingArgs[StrChartSizeFactor]))
        self.graph.show()
    ...
    def initUI(self):
        ...
        for key, value in self.IndicatorParam.items():
            if value[strType] == strComboBox:
                ...
                ComboBox.setCurrentIndex(random.randint(
                    0, len(value[strComboList]) - 1))
