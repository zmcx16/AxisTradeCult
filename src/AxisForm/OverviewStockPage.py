...
class OverviewStockPage(QMainWindow):
    ...
    def RefreshOverviewStock(self):
        ...
        OverviewStocks = ReadOverviewStockData(
            Stocks, QDate.toPyDate(ChooseDateTime), gv.StockDataPoolPath)
        ...
        for stock in OverviewStocks:
            stockInfo = OverviewStockInfoWidget(
                self.parent().ui, stock, OverviewStocks[stock])
            stockInfo.setGeometry(
                QRect(0, 0, stockInfo.widget_width, stockInfo.widget_height))
            stockInfo.setMinimumSize(
                stockInfo.widget_width, stockInfo.widget_height)
    ...
    def DoAddButton(self):
        ...
        gv.AddStockInGroup(
            self.parent().ui.StockGroupsComboBox.currentText(), Symbol)
        self.RefreshOverviewStock()
        self.parent().ui.AddButton.setEnabled(True)
        return True
    ...
    def DoUpdateButton(self):
        ...
        stocksSet = {
            symbol for key in gv.StockGroups for symbol in gv.StockGroups[key]}
        self.parent().ui.UpdateProgressBar.setRange(0, 100)
        self.update_stocks_thread = UpdateStocksThread(stocksSet)
        self.update_stocks_thread.FinishUpdateStocksSignal.connect(
            self.FinishUpdateStocks)
        self.update_stocks_thread.UpdateProgressBarCountSignal.connect(
            self.UpdateProgressBarCount)
...
class OverviewStockInfoWidget(QWidget):
    ...
    def initUI(self, parent, object_id, data):
        ...
        StockCheckBox.setGeometry(QRect(parent.StockCheckBox.x(
        ) + self.offsetX, 0, parent.StockCheckBox.width(), parent.StockCheckBox.height()))
        StockCheckBox.setObjectName(parent.StockCheckBox.objectName() + object_id)
        StockCheckBox.setFont(font)
        OpenLabel = QLabel(self)
        OpenLabel.setGeometry(QRect(parent.OpenLabel.x(
        ) + self.offsetX, 0, parent.OpenLabel.width(), parent.OpenLabel.height()))
        OpenLabel.setAlignment(
            Qt.AlignRight | Qt.AlignTrailing | Qt.AlignVCenter)
        OpenLabel.setObjectName(parent.OpenLabel.objectName() + object_id)
        OpenLabel.setFont(font)
        HightLabel = QLabel(self)
        HightLabel.setGeometry(QRect(parent.HightLabel.x(
        ) + self.offsetX, 0, parent.HightLabel.width(), parent.HightLabel.height()))
        HightLabel.setAlignment(
            Qt.AlignRight | Qt.AlignTrailing | Qt.AlignVCenter)
        HightLabel.setObjectName(parent.HightLabel.objectName() + object_id)
        HightLabel.setFont(font)
        LowLabel = QLabel(self)
        LowLabel.setGeometry(QRect(parent.LowLabel.x(
        ) + self.offsetX, 0, parent.LowLabel.width(), parent.LowLabel.height()))
        LowLabel.setAlignment(
            Qt.AlignRight | Qt.AlignTrailing | Qt.AlignVCenter)
        LowLabel.setObjectName(parent.LowLabel.objectName() + object_id)
        LowLabel.setFont(font)
        CloseLabel = QLabel(self)
        CloseLabel.setGeometry(QRect(parent.CloseLabel.x(
        ) + self.offsetX, 0, parent.CloseLabel.width(), parent.CloseLabel.height()))
        CloseLabel.setAlignment(
            Qt.AlignRight | Qt.AlignTrailing | Qt.AlignVCenter)
        CloseLabel.setObjectName(parent.CloseLabel.objectName() + object_id)
        CloseLabel.setFont(font)
        VolumeLabel = QLabel(self)
        VolumeLabel.setGeometry(QRect(parent.VolumeLabel.x(
        ) + self.offsetX, 0, parent.VolumeLabel.width(), parent.VolumeLabel.height()))
        VolumeLabel.setAlignment(
            Qt.AlignRight | Qt.AlignTrailing | Qt.AlignVCenter)
        VolumeLabel.setObjectName(parent.VolumeLabel.objectName() + object_id)
        VolumeLabel.setFont(font)
        ChangeCLabel = QLabel(self)
        ChangeCLabel.setGeometry(QRect(parent.ChangeCLabel.x(
        ) + self.offsetX, 0, parent.ChangeCLabel.width(), parent.ChangeCLabel.height()))
        ChangeCLabel.setAlignment(
            Qt.AlignRight | Qt.AlignTrailing | Qt.AlignVCenter)
        ChangeCLabel.setObjectName(parent.ChangeCLabel.objectName() + object_id)
        ChangeCLabel.setFont(fontBold)
        ChangeVLabel = QLabel(self)
        ChangeVLabel.setGeometry(QRect(parent.ChangeVLabel.x(
        ) + self.offsetX, 0, parent.ChangeVLabel.width(), parent.ChangeVLabel.height()))
        ChangeVLabel.setAlignment(
            Qt.AlignRight | Qt.AlignTrailing | Qt.AlignVCenter)
        ChangeVLabel.setObjectName(parent.ChangeVLabel.objectName() + object_id)
        ChangeVLabel.setFont(fontBold)
        AvgC3MLabel = QLabel(self)
        AvgC3MLabel.setGeometry(QRect(parent.AvgC3MLabel.x(
        ) + self.offsetX, 0, parent.AvgC3MLabel.width(), parent.AvgC3MLabel.height()))
        AvgC3MLabel.setAlignment(
            Qt.AlignRight | Qt.AlignTrailing | Qt.AlignVCenter)
        AvgC3MLabel.setObjectName(parent.AvgC3MLabel.objectName() + object_id)
        AvgC3MLabel.setFont(font)
        AvgV3MLabel = QLabel(self)
        AvgV3MLabel.setGeometry(QRect(parent.AvgV3MLabel.x(
        ) + self.offsetX, 0, parent.AvgV3MLabel.width(), parent.AvgV3MLabel.height()))
        AvgV3MLabel.setAlignment(
            Qt.AlignRight | Qt.AlignTrailing | Qt.AlignVCenter)
        AvgV3MLabel.setObjectName(parent.AvgV3MLabel.objectName() + object_id)
        AvgV3MLabel.setFont(font)
        StrikePrice1YLabel = QLabel(self)
        StrikePrice1YLabel.setGeometry(QRect(parent.StrikePrice1YLabel.x(
        ) + self.offsetX, 0, parent.StrikePrice1YLabel.width(), parent.StrikePrice1YLabel.height()))
        StrikePrice1YLabel.setAlignment(
            Qt.AlignRight | Qt.AlignTrailing | Qt.AlignVCenter)
        StrikePrice1YLabel.setObjectName(
            parent.StrikePrice1YLabel.objectName() + object_id)
        StrikePrice1YLabel.setFont(font)
        GraphSampleButton = QPushButton(self)
        GraphSampleButton.setEnabled(True)
        GraphSampleButton.setGeometry(QRect(parent.GraphSampleButton.x(
        ) + self.offsetX, 0, parent.GraphSampleButton.width(), parent.GraphSampleButton.height()))
        GraphSampleButton.setObjectName(
            parent.GraphSampleButton.objectName() + object_id)
        ...
        StrikePrice1YLabel.setText(_translate(
            "AxisTradeCultForm", data["StrikePrice1Y"]))
    ...
    def DoGraphButton(self):
        ...
        if GraphType == 'BasicN' or GraphType == 'CandleN':
            response = QInputDialog().getText(None, "Back N Months",
                                              "Please Input Back N Months:")
            if response[1] == True and response[0] != '':
                Back_N_Months = int(response[0])

        TargetDate_back = self.TargetDate - \
            pandas.DateOffset(months=Back_N_Months)
        df = GetStockPriceVolumeData(
            self.Symbol, gv.StockDataPoolPath, TargetDate_back, self.TargetDate)
        ...
        TechIndicators = []
        TechIndicators.append({strName: strMA,             strParam: {
                              strColor: 'blue', strWindow: 20, strLineWidth: 0.8, strAlpha: 0.8}})
        TechIndicators.append({strName: strBollingerBands, strParam: {
                              strColor: 'grey', strWindow: 20, strLineWidth: 0.8, strAlpha: 0.8, strAreaColor: 'gold', strAreaAlpha: 0.3}})
        print(TechIndicators)

        self.graph = ScrollableWindow(PlotStockData(
            self.Symbol, df, PlotType, TechIndicators, gv.SettingArgs[StrChartSizeFactor]))
        self.graph.show()
...
class UpdateStocksThread(QThread):
    ...
    def run(self):
        with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
            future_to_download = {executor.submit(
                DownloadStockDataFromQuandl, symbol, gv.StockDataPoolPath): symbol for symbol in self.stocksSet}
            for future in concurrent.futures.as_completed(future_to_download):
                try:
                    ...
                    if data is True:
                        previous_progress = self.FinishNum / \
                            len(self.stocksSet) * 100
                        self.FinishNum += 1
                        now_progress = self.FinishNum / \
                            len(self.stocksSet) * 100
                        if int(now_progress) != int(previous_progress):
                            self.UpdateProgressBarCountSignal.emit(
                                int(now_progress))
