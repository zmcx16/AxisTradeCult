...
def PlotStockData(Symbol, df_data, PlotType, TechIndicators, size_factor):
    ...
    fig.suptitle('Stock: {0} ({1} ~ {2})'.format(Symbol, df_data.index.min().strftime(
        '%Y-%m-%d'), df_data.index.max().strftime('%Y-%m-%d')), y=1, fontsize=14)
    ...
    row_index[0] = 2
    ax_volume = plt.subplot2grid(
        (row_size, 1), (row_size - 1, 0), sharex=ax_price)
    ...
    if PlotType == strCandle:
        lines, patches = mpl_finance.candlestick_ohlc(
            ax_price, df_data.values, width=1, colorup='r', colordown='g')
    ...
    mpl_finance.volume_overlay(
        ax_volume, df_data[strOpen], df_data[strClose], df_data[strVolume], colorup='r', colordown='g', width=1)
...
def PlotMA(param, df_data, target_ax, row_size, row_index):
    ...
    PlotIndicator(Indicator, target_ax=target_ax, color=param[strColor], linewidth=float(
        param[strLineWidth]), alpha=float(param[strAlpha]))
...
def PlotBollingerBands(param, df_data, target_ax, row_size, row_index):
    ...
    PlotIndicator(IndicatorUpper, target_ax=target_ax, color=param[strColor], linewidth=float(
        param[strLineWidth]), alpha=float(param[strAlpha]))
    PlotIndicator(IndicatorLower, target_ax=target_ax, color=param[strColor], linewidth=float(
        param[strLineWidth]), alpha=float(param[strAlpha]))
    target_ax.fill_between(df_data[strDate], IndicatorUpper, IndicatorLower,
                           interpolate=True, color=param[strAreaColor], alpha=float(param[strAreaAlpha]))


def PlotKDJ(param, df_data, target_ax, row_size, row_index):
    ax = plt.subplot2grid((row_size, 1), (row_index[0], 0), sharex=target_ax)
    row_index[0] += 1
    k, d, j = GetKDJ(df_data[strClose], df_data[strHigh],
                     df_data[strLow], int(param[strWindow]))
    PlotIndicator(k, target_ax=ax, color=param[strKColor], linewidth=float(
        param[strLineWidth]), alpha=float(param[strAlpha]))
    PlotIndicator(d, target_ax=ax, color=param[strDColor], linewidth=float(
        param[strLineWidth]), alpha=float(param[strAlpha]))
    PlotIndicator(j, target_ax=ax, color=param[strJColor], linewidth=float(
        param[strLineWidth]), alpha=float(param[strAlpha]))

    basic_array1 = numpy.ones(len(df_data[strClose]))
    PlotIndicator(pandas.Series(basic_array1 * 80), target_ax=ax,
                  color='grey', linewidth=1.5, alpha=1)
    PlotIndicator(pandas.Series(basic_array1 * 20), target_ax=ax,
                  color='grey', linewidth=1.5, alpha=1)
...
def GetRSV(close_values, high_values, low_values, period):
    hight_max = pandas.Series.rolling(
        high_values, window=period, center=False).max()
    low_min = pandas.Series.rolling(
        low_values, window=period, center=False).min()
    return (close_values - low_min) / (hight_max - low_min) * 100


ColorList = ['aqua', 'aquamarine', 'azure', 'beige', 'black', 'blue', 'brown', 'chartreuse', 'chocolate', 'coral', 'crimson', 'cyan', 'darkblue', 'darkgreen', 'fuchsia', 'gold', 'goldenrod', 'green', 'grey', 'indigo', 'ivory', 'khaki', 'lavender', 'lightblue',
             'lightgreen', 'lime', 'magenta', 'maroon', 'navy', 'olive', 'orange', 'orangered', 'orchid', 'pink', 'plum', 'purple', 'red', 'salmon', 'sienna', 'silver', 'tan', 'teal', 'tomato', 'turquoise', 'violet', 'wheat', 'white', 'yellow', 'yellowgreen']
...
TechIndicatorFuncDict = {
    ...
    strKDJ:               {strFuncName: PlotKDJ,               strParam: {
        strKColor: 'red', strDColor: 'black', strJColor: 'gold', strWindow: 9, strLineWidth: 0.8, strAlpha: 0.8}}
}
...
TechIndicatorWidgetParam = {
    ...
    strKDJ:               {strKColor: {strType: strComboBox,  strValue: 'red', strComboList: ColorList}, strDColor: {strType: strComboBox,  strValue: 'black', strComboList: ColorList}, strJColor: {
        strType: strComboBox,  strValue: 'gold', strComboList: ColorList}, strWindow: {strType: strLineEdit,  strValue: 9},  strLineWidth: {strType: strLineEdit,  strValue: 1}, strAlpha: {strType: strLineEdit,  strValue: 1}}
}
